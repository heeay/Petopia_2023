...
public interface InfoService {
	...
	int checkLike(int infoNo, int userNo);
	
	// 좋아요가 N인지 체크 (N이면 1 반환)
	int checkNoLike(int infoNo, int userNo);
	
	// 좋아요 insert
	int insertLike(Like like);
	
	// 좋아요 삭제
	int deleteLike(int infoNo, int userNo);
	
	// 좋아요 update
	int updateLike(int infoNo, int userNo);
	
	// 좋아요 수
	int countLike(int infoNo);
	
	// 게시글 업데이트
	int updateInfo(Info in, ArrayList<InfoFile> list, int star, int infoNo);
