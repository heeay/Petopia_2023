...
public class AjaxClickLikeController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int check = new InfoServiceImpl().checkLike(map); // Y인 좋아요의 개수
        int checkNo = new InfoServiceImpl().checkNoLike(map); // N인 좋아요의 개수
        
        // System.out.println(check);
        // System.out.println(checkNo);
        
        if(check == 1) { // 게시글에 좋아요가 클릭되어있는 경우 (LIKE_YN == 'Y')
            result = new InfoServiceImpl().deleteLike(map); // LIKE_YN = 'N'으로 변경 (성공 1, 실패 0)
            result += 1; // result + 1 (성공 2)
            
        } else { // check == 0 (LIKE_YN = 'N' OR 좋아요 처음 클릭)
            
            if(checkNo == 1) { // 이전에 좋아요를 클릭했다 취소한 경우 (LIKE_YN == 'N')
                result = new InfoServiceImpl().updateLike(map); // LIKE_YN = 'Y'로 변경
                ...
                result = new InfoServiceImpl().insertLike(like); // 성공 1, 실패 0
            }
        }
