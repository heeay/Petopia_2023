...
/**
 * Servlet implementation class WalkInsertController
 */
@WebServlet("/walkInsert.my")
public class WalkInsertController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public WalkInsertController() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			int result = new PetService().insertWalk(wr, pf);
			
			if(result>0) {
				request.getSession().setAttribute("alertMsg", "산책기록 등록 성공");
				...
				request.setAttribute("errorMsg", "산책기록 등록 실패");
